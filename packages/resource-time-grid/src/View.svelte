<script>
  ...
  import { Header, Body, Day } from "../../time-grid/index";

  import AllDayBody from "../../time-grid/src/AllDayBody.svelte";
  import AllDayOverview from "../../time-grid/src/AllDayOverview.svelte";

  let state = getContext("state");
  let { datesAboveResources, _viewDates, _intlDayHeader, _viewClass, theme } =
    state;
...
<AllDayBody>
  {#each loops[0] as item0}
    <div class={$theme.resource}>
      <AllDayOverview resource={item0} dates={$_viewDates} />
    </div>
  {/each}
</AllDayBody>
